...
/**
 * Global Animation Handler for Shopify Sections
 * Add this to your theme's global.js or create a new file in assets
 * 
 * This handles all scroll-triggered animations across the site
 */

// Global Animation Handler - Works on Load AND Scroll
document.addEventListener('DOMContentLoaded', function() {
  ...
  animatedElements.forEach(function(element) {
    ...
    if (isInViewport(element)) {
      // Animate immediately (with small delay for render)
      setTimeout(function() {
        element.style.animationPlayState = 'running';
      }, 100);
    } else {
      // Set up scroll observer for elements not yet visible
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initializeAnimations() {
    ...
    // Find all animated sections
    const animatedSections = document.querySelectorAll('[class*="floatIn"], [class*="fadeIn"], [class*="slideIn"], [style*="animation"]');
    
    // Also find sections with data attributes for animations
    const dataAnimatedSections = document.querySelectorAll('[data-animation="true"], [data-animate], .animate-on-scroll');
    
    // Combine both NodeLists
    const allAnimatedElements = [...animatedSections, ...dataAnimatedSections];
    
    if (allAnimatedElements.length === 0) return;
    ...
    const animationObserver = new IntersectionObserver(function(entries) {
      entries.forEach(function(entry) {
        if (entry.isIntersecting) {
          ...
          // Check if element has animation style
          const computedStyle = window.getComputedStyle(element);
          const animationName = computedStyle.animationName;
          
          if (animationName && animationName !== 'none') {
            // If animation is paused, play it
          ...
          // For data-attribute based animations
          if (element.hasAttribute('data-animation') || element.hasAttribute('data-animate')) {
  ...
  // Re-initialize when new sections are added (for dynamic content)
...
/**
 * Alternative approach: Specific section animation handler
 * Use this if you want more control over specific animations
 */

function initSectionAnimations() {
  // Target all Shopify sections
  const sections = document.querySelectorAll('[id^="shopify-section-"]');
  
  sections.forEach(function(section) {
    // Look for any animated elements within sections
    ...
    const observer = new IntersectionObserver(
      function(entries) {
        entries.forEach(function(entry) {
          if (entry.isIntersecting) {
            // Find all animated elements in this section
            const section = entry.target.closest('[id^="shopify-section-"]');
            if (section) {
              const toAnimate = section.querySelectorAll('[style*="animation"]');
              toAnimate.forEach(function(elem) {
                elem.style.animationPlayState = 'running';
                elem.style.opacity = ''; // Reset opacity if it was set to 0
              });
            }
            observer.unobserve(entry.target);
          }
    ...
    // Observe the section or its wrapper
    const wrapper = section.querySelector('.wrapper, .content-wrapper, .container');
    const elementToObserve = wrapper || section;
...
// Initialize the specific handler if preferred
